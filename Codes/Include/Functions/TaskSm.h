...
private:
    ...
    std::vector<uchar_t> CalculateClearStream(const MacHeader& dataFrame) const;
    void GuessKeyBytes(uchar_t *iv, size_t ivSize, std::vector<uchar_t>& key, std::vector<uchar_t>& result);
    void Check();
